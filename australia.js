// Get a reference to the element(s) with n-country="australia"
const countryElements = document.querySelectorAll('[n-country="australia"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInAustralia = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === 'australia';
      });

      // Set the display property of the elements based on whether the user is in Australia or not
      countryElements.forEach(element => {
        element.style.display = isInAustralia ? 'flex' : 'none';
      });
    });
}, error => {
