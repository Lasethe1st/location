// Get a reference to the element(s) with n-country="azerbaijan"
const countryElements = document.querySelectorAll('[n-country="azerbaijan"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInAzerbaijan = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === 'azerbaijan';
      });

      // Set the display property of the elements based on whether the user is in Azerbaijan or not
      countryElements.forEach(element => {
        element.style.display = isInAzerbaijan ? 'flex' : 'none';
      });
    });
}, error => {
