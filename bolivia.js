...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      // Check if the user is in Bolivia
      const isInBolivia = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === 'bolivia';
      });

      // Set the display property of the elements based on whether the user is in Bolivia or not
      countryElements.forEach(element => {
        element.style.display = isInBolivia ? 'flex' : 'none';
      });
    });
}, error => {
