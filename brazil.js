// Get a reference to the element(s) with n-country="brazil"
const countryElements = document.querySelectorAll('[n-country="brazil"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInBrazil = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === 'brazil';
      });

      // Set the display property of the elements based on whether the user is in Brazil or not
      countryElements.forEach(element => {
        element.style.display = isInBrazil ? 'flex' : 'none';
      });
    });
}, error => {
