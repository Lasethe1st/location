// Get a reference to the element(s) with n-country="cameroon"
const countryElements = document.querySelectorAll('[n-country="cameroon"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInCameroon = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === "cameroon";
      });

      // Set the display property of the elements based on whether the user is in Cameroon or not
      countryElements.forEach(element => {
        element.style.display = isInCameroon ? 'flex' : 'none';
      });
    });
}, error => {
