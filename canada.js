// Get a reference to the element(s) with n-country="canada"
const countryElements = document.querySelectorAll('[n-country="canada"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInCanada = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === "canada";
      });

      // Set the display property of the elements based on whether the user is in Canada or not
      countryElements.forEach(element => {
        element.style.display = isInCanada ? 'flex' : 'none';
      });
    });
}, error => {
