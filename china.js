// Get a reference to the element(s) with n-country="china"
const countryElements = document.querySelectorAll('[n-country="china"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInChina = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === "china";
      });

      // Set the display property of the elements based on whether the user is in China or not
      countryElements.forEach(element => {
        element.style.display = isInChina ? 'flex' : 'none';
      });
    });
}, error => {
