// Get a reference to the element(s) with n-country="ethiopia"
const countryElements = document.querySelectorAll('[n-country="ethiopia"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInEthiopia = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === "ethiopia";
      });

      // Set the display property of the elements based on whether the user is in Ethiopia or not
      countryElements.forEach(element => {
        element.style.display = isInEthiopia ? 'flex' : 'none';
      });
    });
}, error => {
