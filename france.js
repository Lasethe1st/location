// Get a reference to the element(s) with n-country="france"
const countryElements = document.querySelectorAll('[n-country="france"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInFrance = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === "france";
      });

      // Set the display property of the elements based on whether the user is in France or not
      countryElements.forEach(element => {
        element.style.display = isInFrance ? 'flex' : 'none';
      });
    });
}, error => {
