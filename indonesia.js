// Get a reference to the element(s) with n-country="indonesia"
const countryElements = document.querySelectorAll('[n-country="indonesia"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInIndonesia = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === "indonesia";
      });

      // Set the display property of the elements based on whether the user is in Indonesia or not
      countryElements.forEach(element => {
        element.style.display = isInIndonesia ? 'flex' : 'none';
      });
    });
}, error => {
