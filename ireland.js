// Get a reference to the element(s) with n-country="ireland"
const countryElements = document.querySelectorAll('[n-country="ireland"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInIreland = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === "ireland";
      });

      // Set the display property of the elements based on whether the user is in Ireland or not
      countryElements.forEach(element => {
        element.style.display = isInIreland ? 'flex' : 'none';
      });
    });
}, error => {
