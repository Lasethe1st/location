// Get a reference to the element(s) with n-country="japan"
const countryElements = document.querySelectorAll('[n-country="japan"]');
...
navigator.geolocation.getCurrentPosition(position => {
  ...
  fetch(mapboxAPIUrl)
    ...
    .then(data => {
      ...
      const isInJapan = data.features.some(feature => {
        return feature.place_type.includes('country') && feature.text.toLowerCase() === "japan";
      });

      // Set the display property of the elements based on whether the user is in Japan or not
      countryElements.forEach(element => {
        element.style.display = isInJapan ? 'flex' : 'none';
      });
    });
}, error => {
